...
If you're running in Codex and see a `prose` command, a `*.prose.md` contract (`kind: responsibility`, `kind: function`, `kind: gateway`, `kind: test`, or `kind: pattern`), or a mention of OpenProse, ProseScript, Forme, or Contract Markdown, OpenProse is the language you're being asked to run.
...
Use `prose init` and `prose compose` for progressive program architecture; use
`prose write` for focused authoring when one Contract is already understood.
