...
**A frontier we've started to specify: the source need not be text.** Compile is an intelligent render, and Forme's topology is itself a world-model — so authored intent can arrive as a *typed image*. A diagram of boxes and labelled edges carries exactly the structure that is tedious to write by hand — which node subscribes to which facet, where the fan-out and the diamonds are — and the compile resolve inverts that picture back into `.prose.md` contracts. It's a brief one rung above Markdown, "well-typed" not by a parser but by whether it compiles to an acyclic graph that round-trips stably; the picture *is* the typecheck. Markdown stays the intent: the resolve *emits* contracts a human ratifies, so the model's latitude is quarantined to authoring, exactly where it already lives. This is specified in the OpenProse skill today, not shipped as a runtime path — named here as a direction so it isn't mistaken for a result.
