...
Investigate a topic using web search, document analysis, or other available tools. Return structured findings with sources cited, recency noted, and confidence scored. Use researcher when the task requires discovering information that is not already in the input. This is one of the most common service roles in production systems -- funding scanners, market researchers, competitive mappers, and signal detectors are all researchers.
