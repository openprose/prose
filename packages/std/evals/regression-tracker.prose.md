...
- if no baseline exists for this system: report notes "no baseline — registering this run as initial baseline" and performs set-baseline automatically
...
Persistent agent that maintains the baseline registry. Maps system names to baseline run IDs and their scores.
...
- maintain a compact registry: system_name, baseline_run_id, baseline_scores, timestamp
- when action is "set-baseline": update the registry entry for this system
- when action is "check": return the current baseline without modifying the registry
- when action is "list": return all entries
- keep a history of previous baselines (last 5 per system) for trend analysis
...
- report: the final output matching the system's top-level ensures schema
