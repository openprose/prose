...
describe("command model", () => {
	const supportedCases: Array<[Parameters<typeof canonicalPrompt>, string]> = [
		...
		[["react", []], "prose react"],
		[["react", ["keep an incident briefing current"]], "prose react 'keep an incident briefing current'"],
		[["react", ["surface renewal risks", "--start"]], "prose react 'surface renewal risks' --start"],
		[["write", ["draft a release readiness responsibility"]], "prose write output_mode: source-package-only apply: false run_state: in-context terminal_summary: required interactive: false request: 'draft a release readiness responsibility'"],
		[["write", ["--", "--no-interactive is literal request text"]], "prose write output_mode: source-package-only apply: false run_state: in-context terminal_summary: required interactive: false request: '--no-interactive is literal request text'"],
	...
	const validationCases: Array<[Parameters<typeof canonicalPrompt>, string, string]> = [
		...
		[["react", ["--startup"]], "Unexpected option '--startup'", "prose react [use case...] [--start]"],
		[["react", ["--start", "--start"]], "Duplicate option", "prose react [use case...] [--start]"],
		[["write", []], "Pass text arguments or pipe stdin", "prose write [request...]"],
		[["write", ["   "]], "Pass text arguments or pipe stdin", "prose write [request...]"],
		[["write", ["--interactive", "draft"]], "does not support interactive flags", "prose write [request...]"],
		[["write", ["--no-interactive", "draft"]], "does not support interactive flags", "prose write [request...]"],
