...
`PROSE_ACTIVATION_ID` for quick lookup. Load the referenced compiled intent
from the active OpenProse root before execution, select the matching Forme
manifest when `formeManifestId` is present, then continue as a normal bounded
run.
