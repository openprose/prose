...
- **A responsibility maintaining a world-model** — `vendor-renewals-prepared`
  reads its prior ledger *by reference*, folds in moved signals, and commits the
...
- **Fingerprint-driven skip** — `collect-renewal-signals` carries a watermark as
  *immaterial* state; a re-delivered event does not move `renewal_signals`, so the
  assessor writes a `skipped` receipt and spawns nothing (*cost scales with
  surprise, not the clock*).
- **A `function` helper** — `score-vendor-renewal` is a called, stateless render
  (the library tier), invoked via ProseScript `call`, not a subscribed node.
