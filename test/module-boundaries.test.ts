...
describe("OpenProse module boundaries", () => {
  test("exposes architectural namespaces from the public entry point", () => {
    ...
    for (const removedExport of ["materialize" + "Source", "materialize" + "File"]) {
      expect(removedExport in prose.runtime).toBe(false);
    }
    ...
    expect("providers" in prose).toBe(false);
    expect("providerProtocol" in prose).toBe(false);
