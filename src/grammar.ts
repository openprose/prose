...
export function buildTextMateGrammar(): TextMateGrammar {
  return {
    ...
    repository: {
      ...
      sectionHeader: {
        match:
          "^(###)(\\s+)(Requires|Ensures|Services|Environment|Effects|Access|Execution|Runtime|Strategies|Shape|Errors|Invariants)\\b.*$",
        captures: {
      ...
      prosescript: {
        patterns: [
          {
            match: "\\b(let|call|return|parallel|loop|condition|try)\\b",
